...
n_actions = 15 #len(bids)  # Number of bids to try
#learning_rate = 1e-4
onlyonebid = False # Only openers rebid
twobids = False # rebid + 2 nd response, if False; 3 bids
nopass = False #False # Opener not allowed to pass
#global_clc = 0.5


n_hand = 5 # shape hcp  # Shape+strength len(state)
...
#        self.state = np.array(list(df.iloc[self.ex,4:11])) # North hand always start
        self.state = np.zeros(n_observations)
        self.state[0:5]=np.array(list(df.iloc[self.ex,4:9])) # North hand always start
        self.b1 = self.b2 = -1
...
#        self.state[6] = self.b2
    ...
    def step(self,action):
        ...
        if self.b1 == -1: # first bid b1,rebid
            if nopass:
                action += 1
            if self.state[0] == 5 and self.state[4] <=13: # spades, hcp N
                action = 0 # always P with 13- and only 5 spades
            elif self.state[4] >= 16: # hcp N
                action = 1 # try always 2D with 16+ to see if S learns
            else:
                action += 1 # no pass with 14+ or 6+ spades
            ...
            if action == 0: # pass
                ...
                newbid = min(b2n(self.finalbid) + action,35) #35 = 7N
                self.finalbid = n2b(newbid)

                self.state = np.zeros(n_observations)
                self.state[5:10] = np.array(list(df.iloc[self.ex, 11:16]))  # South hand
                self.state[2*n_hand+action] = 1
...
#                self.state[6] = -1
                ...
                truncated = False
                observation = self.state
        elif self.b2 == -1: # Second bid, 2nd response
            if action == 0: # pass
                reward = imp(result(self.ex,self.finalbid) - self.par) + impofs
                observation = self.state
                terminated = True
                ...
                oldfinal = self.finalbid
                newbid = min(b2n(self.finalbid) + action,35)
#                oldb1 = self.state[(2*n_hand):(2*n_hand+n_actions)]
                self.finalbid = n2b(newbid)
                self.b2 = newbid
                self.state = np.zeros(n_observations)
                self.state[0:5] = np.array(list(df.iloc[self.ex, 4:9]))  # North hand always start
#                self.state = np.array(list(df.iloc[self.ex,4:11])) # norths hand
#                print(oldb1,2*n_hand,2*n_hand+n_actions)
 #               self.state[(2*n_hand):(2*n_hand+n_actions)] = oldb1
                self.state[2*n_hand+self.b1- b2n(oldfinal)] = 1  # Not tested, maybe wrong
                self.state[2*n_hand+n_actions+action] = 1
                if twobids:
                    reward = imp(result(self.ex, self.finalbid) - self.par) + impofs
                    terminated = True
                else:
...
#        print('reset')
    ...
    def render(self):
        ...
        print(result(self.env.ex, self.env.finalbid), self.env.lastreward-impofs, "(", self.env.par, ")", n, s, ns, es, ss, ws )
...
class ActorCritic(nn.Module):
    def __init__(self):
        ...
        self.l1 = nn.Linear(n_observations,100)  #4
        self.l2 = nn.Linear(100,100)
        self.l2b = nn.Linear(100,75)
        self.actor_lin1 = nn.Linear(75,n_actions) #2
        self.l3 = nn.Linear(75,50)
...
#        critic = torch.tanh(self.critic_lin1(c))
        critic = 20*torch.tanh(self.critic_lin1(c)) # imp diffs in [-20,+20}
        return actor, critic
...
def worker(t, worker_model, counter, params):
    ...
    for i in range(params['epochs']):
        ...
        #print(f"worker:{values=}{logprobs=}{rewards=}")
...
def run_episode(worker_env, worker_model):
    ...
    while (done == False):
        ...
        #print(f"Runepisode:{state_}{r=}{done=}")
...
def update_params(worker_opt, values, logprobs, rewards, clc=0.1, gamma=0.95):
    ...
    Returns = []
    ret_ = torch.Tensor([0])
    #print(f"up3:{ret_=}rs={rewards.shape[0]}")
    for r in range(rewards.shape[0]):
        ret_ = rewards[r] + gamma * ret_
        #print(f"up4:{ret_=}")
        Returns.append(ret_)
    Returns = torch.stack(Returns).view(-1)
    #Returns = f.normalize(Returns, dim=0) # Don't normalize one return
    #print(f"u_p:{rewards=}{logprobs=}{values=}{Returns=}")
    actor_loss = -1*logprobs * (Returns - values.detach())
    critic_loss = torch.pow(values - Returns, 2)
    loss = actor_loss.sum() + global_clc*critic_loss.sum()
...
def test():
    ...
    imps=0
    contracts = 0
    print("1♠1N-Bidding Score Diff (Par) N[shape,hcp] S[shape,hpc]  N-cards E-cards S-cards W-cards")
    for i in range(1000): # Run bids for statistics but only print 100
...
#        print(f"{action_dist=}")
        ...
        #print("test:",state2,state,action.detach().numpy(),reward,done,env.env.finalbid)
        if done:
            #print()
            #print(state2,reward)
            imps += reward
        ...
        state_ = np.array(env.env.state)
        state = torch.from_numpy(state_).float()
    print(f"Average diff over 1000 bids: {imps/contracts-impofs:.3}")
# --- End of DA2C part
